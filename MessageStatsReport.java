...
public class MessageStatsReport extends Report implements MessageListener {
	...
	private double lastReportTime;        //  to store the creation time of the last report

    private HashMap<Message, Double> EndtoEndDelaysTable;
	...
	protected void init() {
        ...
        EndtoEndDelaysTable = new HashMap<>();
    }
	...
	public void messageTransferred(Message m, DTNHost from, DTNHost to,
		...
		if (finalTarget) {
			...
			if(EndtoEndDelaysTable.containsKey(m)){
			    double value = EndtoEndDelaysTable.get(m);
			    double delay = m.getReceiveTime() - m.getCreationTime();
			    EndtoEndDelaysTable.put(m,value+delay);
            }
            else{
                double delay = m.getReceiveTime() - m.getCreationTime();
                EndtoEndDelaysTable.put(m,delay);
            }
	...
	private double calculateEndToEndDelay(){
        double endToEndDelay = 0.0;

	    for(Map.Entry<Message,Double> entry : EndtoEndDelaysTable.entrySet()){
	        endToEndDelay += entry.getValue();
        }
    ...
    protected void newEvent() {
        ...
        if (getSimTime() > lastReportTime + 3600) {
            ...
            double totalEndToEndDelay = calculateEndToEndDelay();
            ...
            String statsText = "created: " + this.nrofCreated +
                    ...
                    "\nEnd-to-End delay: " + totalEndToEndDelay/this.nrofDelivered +
                    "\n----------------------------------"
                    ;
