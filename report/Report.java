...
public abstract class Report {
    ...
    public static final String NUMHOPS = "k";
    public int K;
	...
	public Report(){
		...
		if(settings.contains(NUMHOPS)){
		    K = settings.getInt(NUMHOPS);
        }else{
            System.out.println("K value not given. Taking default value = 3");
            K = 3;
        }
		...
		if (settings.contains(OUTPUT_SETTING)) {
			...
			outFileName = outDir + scenarioName + 
				"_" + this.getClass().getSimpleName() + "_" + K;
			if (outputInterval == -1) {
